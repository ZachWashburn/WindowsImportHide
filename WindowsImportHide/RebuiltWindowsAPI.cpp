...
namespace RebuiltWindowsAPI {
	...
	FARPROC WINAPI GetExportAddress(_In_ HMODULE hModule, _In_ LPCSTR lpProcName, _In_ BOOLEAN MappedAsImage)
	{
		...
		//static WINDOWS_IMPORT_HIDE(LoadLibraryA, "kernel32.dll");
		if (lpProcName == NULL)
			return nullptr;
		...
		for (DWORD i = 0; i < ExportDirectory->NumberOfFunctions; i++)
		{
				...
				ExportFnc < ((BYTE*)ExportDirectory + ExportDirectorySize))
			{
				...
				if (ForwardedFunctionName)
				{
					...
					HMODULE hForwardedDll = LoadDll(szForwardedLibraryName);
	...
	//////////////////////////////////////////////////////////////////////////////////////////////////
	//		Equivalent to the windows api function GetProcAddress, But Using a hashed proc name
	//////////////////////////////////////////////////////////////////////////////////////////////////
	/// <summary>
	/// Retrieves the address of an exported function inside the specified module, takes an FNV Hash instead of an lpProcName
	/// </summary>
	/// <param name="hModule">Address of the module</param>
	/// <param name="ulProcNameHash">Hashed Name Of Exported Function</param>
	/// <param name="MappedAsImage">Is the module mapped or a raw file? (TRUE / FALSE)</param>
	/// <returns>returns the exported procedure address inside the specified module</returns>
	FARPROC WINAPI GetExportAddressByHash(_In_ HMODULE hModule, _In_ unsigned long ulProcNameHash, _In_ BOOLEAN MappedAsImage)
	{

		//static WINDOWS_IMPORT_HIDE(LoadLibraryA, "kernel32.dll");
		if (ulProcNameHash == NULL)
			return nullptr;
		...
		for (DWORD i = 0; i < ExportDirectory->NumberOfFunctions; i++)
		{
				...
				ExportFnc < ((BYTE*)ExportDirectory + ExportDirectorySize))
			{
				...
				if (ForwardedFunctionName)
				{
					if (ulProcNameHash != fnv::hashRuntime(ForwardedFunctionName))
						continue;


					HMODULE hForwardedDll = LoadDll(szForwardedLibraryName);
					FARPROC ForwardedFunction = (FARPROC)GetExportAddressByHash(hForwardedDll, ulProcNameHash, MappedAsImage);
					return (FARPROC)ForwardedFunction;
				}
			...
			if (ulProcNameHash == fnv::hashRuntime(ProcNamePtr))
				return (FARPROC)ExportFnc;
	...
	typedef NTSTATUS(__stdcall* fnLdrLoadDll_t)(
		IN __int16 _SearchPath  	OPTIONAL,
		IN PULONG DllCharacteristics  	OPTIONAL,
		IN PUNICODE_STRING  	DllName,
		OUT PVOID* BaseAddress
	);

	typedef BOOLEAN(__stdcall* fnRtlCreateUnicodeString_t)(
		PUNICODE_STRING DestinationString,
		PCWSTR          SourceString
	);

	typedef BOOLEAN(__stdcall* fnRtlFreeUnicodeString_t)(
		PUNICODE_STRING DestinationString
		);
	...
	std::string find_file_path_recursive(std::string fileName, std::string szPath)
	{
		if (DoesFileExist(fileName)) // first off quickly verify this ain't the case
			return ".\\";

		std::filesystem::path filename(fileName); // exclude any path
		std::filesystem::recursive_directory_iterator i(szPath,
			...
			try {
				for (; i != end; ++i)
					if (!is_directory(i->path()))
						if (!lstrcmpA(i->path().filename().string().c_str(), filename.filename().string().c_str()))
	...
	HMODULE LoadDll(const char* szDLLName)
	{

		//return LoadLibraryA(szDLLName);

		RBUILTWINAPISTATIC HMODULE ntdll = RebuiltWindowsAPI::GetModuleA(XorStr("ntdll.dll"));
		RBUILTWINAPISTATIC FARPROC LdrLoadDll = RebuiltWindowsAPI::GetExportAddress(ntdll, XorStr("LdrLoadDll"));
		RBUILTWINAPISTATIC FARPROC LdrUnloadDll = RebuiltWindowsAPI::GetExportAddress(ntdll, XorStr("LdrUnloadDll"));
		RBUILTWINAPISTATIC FARPROC LdrGetDllHandle = RebuiltWindowsAPI::GetExportAddress(ntdll, XorStr("LdrGetDllHandle"));
		RBUILTWINAPISTATIC FARPROC LdrGetProcedureAddres = RebuiltWindowsAPI::GetExportAddress(ntdll, XorStr("LdrGetProcedureAddress"));
		RBUILTWINAPISTATIC FARPROC RtlCreateUnicodeString = RebuiltWindowsAPI::GetExportAddress(ntdll, XorStr("RtlCreateUnicodeString"));	
		RBUILTWINAPISTATIC FARPROC RtlFreeUnicodeString = RebuiltWindowsAPI::GetExportAddress(ntdll, XorStr("RtlFreeUnicodeString"));
		
		HINSTANCE hInst;
		UNICODE_STRING dllName;

		char Buffer[8192];
		// let it allocate the buffer to be extra safe maybe?
		_getcwd(Buffer, sizeof(Buffer));

		std::string szPath = find_file_path_recursive(szDLLName, Buffer);
		
		if (szPath.length() < 2)
		{
			//szPath = find_file_path_recursive(szDLLName, std::string(XorStr("C:\\Windows\\WinSxS\\")));
		}


		std::string filename = szDLLName;
		std::wstring wpath(szPath.begin(), szPath.end());
		std::wstring name(filename.begin(), filename.end());

		((fnRtlCreateUnicodeString_t)RtlCreateUnicodeString)(&dllName, name.c_str());

		auto Status = ((fnLdrLoadDll_t)LdrLoadDll)((__int16)(PWSTR)wpath.c_str(),
			NULL,
			&dllName,
			(PVOID*)&hInst);

		((fnRtlFreeUnicodeString_t)RtlFreeUnicodeString)(&dllName);
		return hInst;
